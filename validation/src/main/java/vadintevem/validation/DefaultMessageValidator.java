...
public class DefaultMessageValidator implements MessageValidator {
    ...
    private static Validation<String, Message> notNullOrEmpty(Message message) {
        return message.getContent() != null && message.getContent().length() > 0
                ? success(message)
                : failure("Message cannot be empty");
    }

    private static Validation<String, Message> noLongerThan(int limit, Message message) {
