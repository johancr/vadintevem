...
SERVICE=vadintevem-service
WAR=gce-events-1.0-SNAPSHOT.war
# [END useful]
...
function print_usage() {
  ...
  echo "deploying the app.  You may add all your configuration here, so you don't need"
...
gce)
  set -v
# [START gce-single]
  mvn clean package -pl gce-events -am

  gsutil cp -r gce-events/target/${WAR} gce/base gs://${BUCKET}/gce/
  ...
  gcloud compute instances create gce-events-instance \
...
down)
  set -v
  gcloud compute instances delete gce-events-instance --zone=${ZONE}
